...
router.post("/conversation", async (req, res) => {
  try {
    ...
    console.log('Processing conversation:', { sessionId, message: message.substring(0, 50) + '...', selectedDocument });
    ...
    if (!useQdrant) {
      ...
      return await handleInMemoryConversation(req, res, sessionId, message, selectedDocument);
    }
...
async function handleInMemoryConversation(req, res, sessionId, message, selectedDocument) {
...
router.get("/sessions", async (req, res) => {
  try {
    ...
    for (const session of sessions) {
      // The sessionId is in metadata, not directly on the object
      const sessionId = session.metadata?.sessionId || session.payload?.sessionId;
      console.log('Processing session:', sessionId, 'from object:', session.id);
    ...
    for (const conversation of conversationResults) {
      // The sessionId and other fields are in metadata
...
router.delete("/sessions/:sessionId", async (req, res) => {
  try {
    ...
    // Search for session to get point IDs
    const sessionResults = await qdrant.searchInCollection("sessions", new Array(768).fill(0), 100, {
      sessionId: sessionId
    }, 0.0);

    // Search for conversations to get point IDs
    const conversationResults = await qdrant.searchInCollection("conversations", new Array(768).fill(0), 1000, {
      sessionId: sessionId
    }, 0.0);
    ...
    for (const session of sessionResults) {
      try {
        await qdrant.client.delete("sessions", {
          wait: true,
          points: [session.id]
        });
      } catch (deleteError) {
        console.error(`Error deleting session point ${session.id}:`, deleteError);
      }
    }

    // Delete conversation points
    for (const conversation of conversationResults) {
      try {
        await qdrant.client.delete("conversations", {
          wait: true,
          points: [conversation.id]
        });
      } catch (deleteError) {
        console.error(`Error deleting conversation point ${conversation.id}:`, deleteError);
      }
    }

    console.log(`Session ${sessionId} and ${conversationResults.length} conversations deleted`);

    res.json({
      message: `Session ${sessionId} deleted successfully`,
      deletedConversations: conversationResults.length
    });
